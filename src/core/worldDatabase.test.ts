...
describe('world database asset URL', () => {
  test.each([
    ['/', '/data/world/pl-2026-v1.json'],
    ['/my_football_legend/', '/my_football_legend/data/world/pl-2026-v1.json'],
  ])('resolves Vite BASE_URL %s', (baseUrl, expected) => {
    expect(buildWorldDatabaseUrl(baseUrl)).toBe(expected);
  });
});
