...
const weighted = (attrs: PlayerAttributes, keys: (keyof PlayerAttributes)[]) => keys.reduce((s,k)=>s+attrs[k],0)/keys.length;
...
export const testTier = (career: CareerState, event: EventInstance, decisionId: string, attrs: (keyof PlayerAttributes)[], difficulty = 62): ResolutionTier => {
  const rng = RandomGenerator.import(event.randomState).fork(decisionId); const random = rng.int(-18, 18);
  const context = (career.player.morale - 50) * .18 + (career.player.fitness - 50) * .16 + (career.historyFacts.some(f=>f.tags.includes('coach_first_impression')) ? 2 : 0);
  return tierFor(weighted(career.player.attributes, attrs) + context + random - (difficulty - 60));
};
export const forceTierForTest = (score: number) => tierFor(score);
const fact = (career: CareerState, event: EventInstance, decisionId: string, type: string, objectiveOutcome: string, tier: ResolutionTier, causes: string[] = []): HistoryFact => ({ id: `fact_${event.definitionId}_${decisionId}`, factType: type, season: career.currentSeason, date: String(event.context.date ?? '2026-07-01'), actors: [career.player.id], targets: event.definitionId === 'academy_coach_introduction' ? [COACH_ID] : [RIVAL_ID], clubs: [career.currentClub.id], competitions: [], data: { decisionId, objectiveOutcome, resolutionTier: tier }, causes, tags: ['academy_first_week', ...event.definitionId === 'academy_first_scrimmage' ? ['first_scrimmage'] : event.definitionId === 'academy_coach_introduction' ? ['coach_first_impression'] : ['first_rival']], visibility: 'partial', narrativeImportance: event.definitionId === 'academy_first_scrimmage' ? 70 : 55, emotionalTone: tier.includes('Failure') ? 'negative' : tier === 'mixed' ? 'bittersweet' : 'positive' });
export const resolveEventChoice = (career: CareerState, decision: EventDecision): EventResolution => {
  const event = career.activeEvent!;
  if (event.definitionId === 'academy_coach_introduction') {
  ...
  if (event.definitionId === 'academy_first_scrimmage') {
