...
import {
  ACADEMY_ANCHORS,
  ACADEMY_EVENT_POOL,
  buildAcademySequence,
  selectCareerEventsForWeek,
} from './academyEventPool';
import type { CareerWeek } from '../../types/domain';

const career = (seed: string, position = 'winger') =>
  createCareerState(
...
describe('academy event pool', () => {
  ...
  it('respects goalkeeper position and once-per-career facts', () => {
    ...
    const blocked = {
      ...career('once'),
      historyFacts: [
        ...career('once').historyFacts,
        {
          id: 'done',
          factType: 'career_event_academy_technical_test',
          season: 2026,
          date: '2026-07-01',
          actors: [],
          targets: [],
