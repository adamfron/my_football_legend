...
import { advanceCareerWeek, generateFixtureSchedule, getCurrentCareerWeek, initializeCurrentCareerWeek, shouldScheduleOffFieldEvent } from './careerWeeks';
import { careerStateSchema } from '../schemas/domainSchemas';

const career = (seed = 'week-test') => {
  const profile = generateStartingPlayerProfile({ firstName: 'Jan', lastName: 'Nowak', nationality: 'PL', age: 16, dominantFoot: 'right', position: 'central_midfielder', heightCm: 178, weightKg: 70, seed }, seed, 0);
  const base = createCareerState(profile, seed);
  return { ...base, historyFacts: [...base.historyFacts, { id: 'old-september-complete', factType: 'september_2026_completed', season: 2026, date: '2026-09-30', actors: [base.player.id], targets: [], clubs: [base.currentClub.id], competitions: [], data: {}, causes: [], tags: [], visibility: 'public' as const, narrativeImportance: 60, emotionalTone: 'neutral' as const }] };
};
...
describe('reusable career week loop', () => {
  it('creates an idempotent schedule with fixture and rest weeks', () => {
    ...
    expect(first.careerCalendar?.weeks).toHaveLength(13);
  ...
  it('crosses month boundaries and advances twelve weeks without duplicates', () => {
    ...
    expect(new Set(state.careerCalendar?.weeks.map((week) => week.id)).size).toBe(13);
  ...
  it('generates deterministic unique fixtures and spaced events', () => {
    ...
    for (let index = 0; index < 12; index++) { decisions.push(shouldScheduleOffFieldEvent(state, getCurrentCareerWeek(state)!)); state = advanceCareerWeek(state); }
