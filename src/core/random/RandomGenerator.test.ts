...
describe('RandomGenerator', () => {
  it('returns the same sequence for the same seed', () => {
    ...
    expect([a.float(), a.int(1, 10), a.bool(0.4), a.pick(['x', 'y'])]).toEqual([b.float(), b.int(1, 10), b.bool(0.4), b.pick(['x', 'y'])]);
  });
  ...
  it('supports weighted picks, shuffling and independent forks', () => {
    ...
    expect(rng.weighted([{ item: 'a', weight: 0 }, { item: 'b', weight: 2 }])).toBe('b');
    expect(rng.shuffle([1, 2, 3, 4]).sort()).toEqual([1, 2, 3, 4]);
    expect(RandomGenerator.fromSeed('tools').fork('child').float()).toBe(RandomGenerator.fromSeed('tools').fork('child').float());
  });
});
