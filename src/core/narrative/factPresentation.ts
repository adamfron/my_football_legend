...
export const getFactPresentation = (career: CareerState, fact: HistoryFact): FactPresentation => {
  ...
  else if (fact.factType === 'academy_training_focus') pair = ['Kierunek przygotowań', `Po informacji zwrotnej od ${coachName(career)} wybrałeś przygotowanie oparte o ${String(fact.data.strongAttributeName ?? 'mocny element')} i pracę wokół ${String(fact.data.developmentAttributeName ?? 'obszar rozwoju')}.`];
  else if (fact.factType === 'academy_rival_preparation') pair = ['Dodatkowe zajęcia z konkurentem', `Decyzja po zajęciach zmieniła układ z ${rivalName(career)} bez robienia z niego ani sojusznika, ani wroga na stałe.`];
  else if (fact.factType === 'academy_final_assessment') pair = ['Końcowy sprawdzian', `W grze kontrolnej pokazałeś reakcję na sytuację, która odbiegała od planu. ${coachName(career)} oceniał cały kontekst dwóch tygodni.`];
  else if (fact.factType === 'academy_selection_result') { const outcome = String(fact.data.selectionOutcome); pair = outcome === 'player_invited' ? ['Zaproszenie do seniorów', `${coachName(career)} zaprosił cię na próbny trening seniorów.`] : outcome === 'both_invited' ? ['Wspólna szansa wyżej', `${coachName(career)} zaprosił ciebie i ${rivalName(career)} na próbę z seniorami.`] : outcome === 'rival_invited_player_plan' ? ['Indywidualny plan rozwoju', `${rivalName(career)} dostał pierwszą szansę, a ty konkretny plan i następną okazję.`] : ['Dodatkowy sprawdzian akademii', `${coachName(career)} nie zamknął wyboru i skierował was do dodatkowego meczu akademii.`]; }
  else if (fact.factType === 'academy_selection_response') pair = ['Odpowiedź na decyzję', `Twoja reakcja na słowa trenera dopisała nowy ton do relacji z ${coachName(career)}.`];
  else if (fact.factType === 'academy_second_week_completed') pair = ['Dwa tygodnie domknięte', `Akademia ${career.currentClub.name} ma już zapis twoich decyzji, relacji i kolejnego kroku kariery.`];
