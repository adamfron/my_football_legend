...
export const createCompletedSeasonSnapshot = (career: CareerState): CompletedSeasonSnapshot => {
  ...
  const fixtures = (career.matchHistory ?? [])
    .filter((m) => m.date >= season.startDate && m.date <= season.endDate)
    .map((m) => ({ ...m }));
  const sum = (key: keyof MatchAppearance) =>
    fixtures.reduce((n, m) => n + (typeof m[key] === 'number' ? Number(m[key]) : 0), 0);
  const ratings = fixtures.flatMap((m) => (m.rating === undefined ? [] : [m.rating]));
  const ledger = career.seasonParticipation ?? [];
  const participation = getParticipationTotals(ledger);
  ...
  return {
    ...
    player: {
      appearances: ledger.length
        ? participation.appearances
        : fixtures.filter((m) => m.minutes > 0).length,
      starts: ledger.length ? participation.starts : fixtures.filter((m) => m.started).length,
      ...
      averageRating: ledger.length
        ? participation.averageRating
        : ratings.length
          ? ratings.reduce((a, b) => a + b, 0) / ratings.length
          : 0,
      yellowCards: sum('yellowCards'),
      redCards: fixtures.filter((m) => m.redCard).length,
