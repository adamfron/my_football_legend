...
export const rollOverClubWorld = (clubs: ProfessionalClub[], seed: string): ProfessionalClub[] => {
  ...
  for (let tier = 1; tier <= 4; tier++) {
    const ranked = clubs.filter(c => c.leagueTier === tier).sort((a, b) => {
      const score = (club: ProfessionalClub) => club.overallStrength + RandomGenerator.fromSeed(`${seed}:${tier}:${club.id}`).int(-12, 12);
...
export const validateClubWorld = (clubs: ProfessionalClub[]): boolean =>
  clubs.length === 48 && new Set(clubs.map(c => c.id)).size === 48 &&
  [1, 2, 3, 4].every(tier => clubs.filter(c => c.leagueTier === tier).length === 12);
