...
export const buildWorldDatabaseUrl = (baseUrl: string): string => {
  const normalizedBaseUrl = `/${baseUrl}`.replace(/\/{2,}/g, '/').replace(/\/$/, '');
  return `${normalizedBaseUrl}/data/world/${WORLD_DATABASE_VERSION}.json`;
};
...
export const loadWorldDatabase = async (): Promise<WorldDatabase> => {
  ...
  if (!response.ok) throw new Error(`Nie udało się pobrać świata z ${url} (${response.status}).`);
