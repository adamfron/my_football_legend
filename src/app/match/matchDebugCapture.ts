...
export const debugFrameSchema = z.object({
  ...
  ball: z.object({
    ...
    bounceCount: z.number().int().optional(),
    launchSpeed: z.number().optional(),
    launchElevation: z.number().optional(),
  ...
  players: z.array(
    z.object({
      ...
      vy: z.number(),
      target: pointSchema,
      locomotionIntensity: z.string().optional(),
      locomotionReason: z.string().optional(),
      speed: z.number(),
      targetSpeed: z.number().optional(),
      locomotionTelemetry: z.unknown().optional(),
    ...
    (() => {
      ...
      return {
        ...
        ball: {
          ...
          vz: state.ball.velocity?.z ?? 0,
          flightTime: state.ball.flightTime,
          distanceTravelled: state.ball.distanceTravelled,
          bounceCount: state.ball.bounceCount,
          launchSpeed: state.ball.launchSpeed,
          launchElevation: state.ball.launchElevation,
        ...
        players: state.players.map((p) => ({
          ...
          vy: p.velocity.y,
          target: p.target,
          locomotionIntensity: p.locomotionIntensity,
          locomotionReason: p.locomotionReason,
          speed: Math.hypot(p.velocity.x, p.velocity.y),
          targetSpeed: p.targetSpeed,
          locomotionTelemetry: p.locomotionTelemetry,
